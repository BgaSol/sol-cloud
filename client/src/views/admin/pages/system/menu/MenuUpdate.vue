...
import {ref} from "vue";
import {MenuEntity, MenuUpdateDto, Service, VerificationResult} from "~/generated/system";
...
const queryUpdateData = async () => {
  return Service.findByIdMenuController(props.id, false).then((res) => {
    if (res.code === 200 && res.data) {
      data.value = {
        ...
        name: res.data?.name || '',
        menuType: res.data?.menuType || MenuEntity.menuType.MENU,
        path: res.data?.path || '',
        icon: res.data?.icon || '',
        routeName: res.data?.routeName || '',
        buttonCode: res.data?.buttonCode || '',
        isExternal: res.data?.isExternal || false,
        externalUrl: res.data?.externalUrl || '',
        isExternalOpen: res.data?.isExternalOpen || false,
        isDisabled: res.data?.isDisabled || false,
        isHidden: res.data?.isHidden || false,
        menuGroup: res.data?.menuGroup || '',
      }
    }
  });
}

const defaultData: () => MenuUpdateDto = () => ({
  id: '',
  sort: 0,
  description: '',
  parentId: '',
...
const data = ref<MenuUpdateDto>(defaultData());
...
const openDialog = () => {
  ...
  Promise.all([queryUpdateData(), getMenuTree()]).then(() => {
    visible.value = true;
  }).finally(() => {
...
const submitForm = () => {
  ...
  Service.applyMenuController(buildDto(defaultData(), data.value) as MenuUpdateDto).then((res) => {
    if (res.code === 400) {
...
const getMenuTree = async () => {
  return Service.findAllMenuController(false).then((res) => {
    menuTree.value = (res.data as MenuEntity[]).filter(item => item.id !== props.id);
  });
}
</script>
...
<template>
  ...
  <el-dialog v-model='visible' append-to-body draggable title='修改菜单' width='700'>
